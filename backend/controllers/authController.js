...
exports.forgotPassword = catchAsyncErrors( async (req, res, next) => {
    ...
    const message = `Mã đặt lại mật khẩu:\n\n${resetUrl}\n\nNếu không yêu cầu có thể bỏ qua.`

    try {

        await sendEmail({
            email: user.email,
            subject: `Shopphone đã khôi phục mật khẩu của bạn.`,
            message
        })

        res.status(200).json({
            success: true,
            message: `Đã gửi tới email: ${user.email}`
        })
...
exports.resetPassword = catchAsyncErrors( async (req, res, next) => {
    ...
    if(!user) {
        return next(new ErrorHandler('Mã thay đổi password không tồn tại hoặc hết hạn', 400))
    }
...
// Updete user profile  => /api/v1/me/update
...
exports.getUserDetails = catchAsyncErrors(async (req, res, next) => {
    ...
    if(!user) {
        return next(new ErrorHandler(`Không tìm thấy người dùng với id: ${req.params.id}`))
    }
